...
var utils = {
        ...
        roles.forEach(function (roleName) {
            ...
            if (Array.isArray(role.$extend)) {
                var rolesMetCondition = role.$extend.filter(function (roleCondition) {
                    return skipConditions || condtions_1.conditionEvaluator(roleCondition.condition, context);
    ...
    getUnionResourcesOfRoles: function (grants, role) {
        ...
        // get roles and extended roles in a flat array
        var roles = utils.getFlatRoles(grants, role, undefined, true);
        // iterate through roles and add permission attributes (array) of
        // each role to attrsList (array).
        return roles.filter(function (role) {
            return grants[role] && grants[role].grants;
    ...
    areExtendingRolesAllowing: function (roleConditions, allowingRoles, query) {
        if (!roleConditions) {
            return false;
        }
        return roleConditions.some(function (roleCondition) {
            return allowingRoles[roleCondition.role] &&
                (query.skipConditions || condtions_1.conditionEvaluator(roleCondition.condition, query.context));
        });
    },
