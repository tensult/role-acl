...
var utils = {
    ...
    getUnionGrantsOfRoles: function (grants, role, context) {
        if (!grants) {
            throw new core_1.AccessControlError('Grants are not set.');
        }
        // get roles and extended roles in a flat array
        var roles = utils.getFlatRoles(grants, role, context, !context);
        // iterate through roles and add permission attributes (array) of
        // each role to attrsList (array).
        return roles.filter(function (role) {
            return grants[role] && grants[role].grants;
    ...
    getUnionResourcesOfRoles: function (grants, role, context) {
        return utils.getUnionGrantsOfRoles(grants, role, context)
            .filter(function (grant) {
            return !context || condtions_1.conditionEvaluator(grant.condition, context);
        }).map(function (grant) {
            return utils.toStringArray(grant.resource);
        }).reduce(Notation.Glob.union, []);
    },
    getUnionActionsOfRoles: function (grants, role, resource, context) {
