...
var utils = {
    anyMatch: function (strings, patterns) {
        var stringArray = utils.toStringArray(strings);
        var patternArray = utils.toStringArray(patterns);
        ...
        roles.forEach(function (roleName) {
            ...
            if (roleItem.$extend) {
                var rolesMetCondition = Object.keys(roleItem.$extend).filter(function (role) {
                    return skipConditions || conditions_1.conditionEvaluator(roleItem.$extend[role].condition, context);
                });
    ...
    getUnionGrantsOfRoles: function (grants, query) {
        ...
        return roles.filter(function (role) {
            ...
            return query.skipConditions || conditions_1.conditionEvaluator(grant.condition, query.context);
        });
    },
    ...
    areGrantsAllowing: function (grants, query) {
        ...
        return grants.some(function (grant) {
            return utils.anyMatch(query.resource, grant.resource)
                && utils.anyMatch(query.action, grant.action)
                && (query.skipConditions || conditions_1.conditionEvaluator(grant.condition, query.context));
        });
    },
    areExtendingRolesAllowing: function (roleExtensionObject, allowingRoles, query) {
        ...
        return Object.keys(roleExtensionObject).some(function (role) {
            return allowingRoles[role] &&
                (query.skipConditions || conditions_1.conditionEvaluator(roleExtensionObject[role].condition, query.context));
        });
    },
